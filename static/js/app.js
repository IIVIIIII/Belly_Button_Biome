d3.json("https://raw.githubusercontent.com/IIVIIIII/Belly_Button_Biome/main/resources/samples.json").then(data => {
    ...
    var bubbleSample = data.samples.filter(s => s.id == data.names[0])[0];
    ...
    var subject = Object
            .entries(data.metadata.filter(s => s.id == data.names[0])[0])
    ...
    var gaugeTrace = {
        domain: { x: [0, 1], y: [0, 1] },
        value: data.metadata.filter(s => s.id == data.names[0])[0].wfreq,
